...
class RemoteSimulate(ConfluxTestFramework):
    ...
    def _test_quantum_sign(self):
        ...
        # generate accounts
        sec_key_list = self.generate_quantum_accounts(ACCOUNT_NUM)
        # print("sec_key_list:", sec_key_list)
        ...
        for address, pub_key in address_list.items():
            ...
            thread = threading.Thread(target=self.sign_task, args=(address, pub_key, "test_quantum_sign", secrete_key, index))
        ...
        for key, value in address_list.items():
            thread = threading.Thread(target=self.sign_task, args=(key, value, "test_quantum_sign", ""))
            thread.start()
